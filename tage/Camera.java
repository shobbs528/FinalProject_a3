...
public class Camera
{
	...
	public void cameraYaw(float x)
	{
		u.rotateAxis((float) Math.toRadians(x), v.x, v.y, v.z);
		n.rotateAxis((float) Math.toRadians(x), v.x, v.y, v.z);
	}

	public void cameraPitch(float x)
	{
		v.rotateAxis((float) Math.toRadians(x), u.x, u.y, u.z);
		n.rotateAxis((float) Math.toRadians(x), u.x, u.y, u.z);
	}

	public void moveFrontBack(float x) {
		Vector3f oldPosition = getLocation();
		Vector3f fwdDirection = getN();
		fwdDirection.mul(x);
		Vector3f newPosition = oldPosition.add(fwdDirection.x(), fwdDirection.y(), fwdDirection.z());
		setLocation(newPosition);
	}

	public void cameraPanSide(float x)
	{
		Vector3f oldPosition = getLocation();
		Vector3f sideDirection = getU();
		sideDirection.mul(x);
		Vector3f newPosition = oldPosition.add(sideDirection.x(), sideDirection.y(), sideDirection.z());
		setLocation(newPosition);
	}

	public void cameraPanVert(float x)
	{
		Vector3f oldPosition = getLocation();
		Vector3f moveDirection = getV();
		moveDirection.mul(x);
		Vector3f newPosition = oldPosition.add(moveDirection.x(), moveDirection.y(), moveDirection.z());
		setLocation(newPosition);
	}
	...
	public Matrix4f getViewR() { return viewR; }
	public void setViewR(Matrix4f m) { viewR = m; }
	public void setView(Matrix4f m) { view = m; }
